-- Migration to add focus_history to revision_problems and update sync_focus_event RPC

-- Add focus_history column if not exists
ALTER TABLE public.revision_problems ADD COLUMN IF NOT EXISTS focus_history JSONB DEFAULT '[]'::jsonb;

-- Re-create sync_focus_event to append attempts to focus_history
...
BEGIN
  ...
  IF v_existing_id IS NOT NULL THEN
    ...
    SET title = COALESCE(NULLIF(btrim(p_title), ''), title),
        ...
        updated_at = NOW(),
        focus_history = COALESCE(focus_history, '[]'::jsonb) || jsonb_build_array(v_new_attempt)
  ...
  ELSE
    INSERT INTO public.revision_problems (
      ...
      p_code,
      NOW(),
      jsonb_build_array(v_new_attempt)
    )
    ...
    SET title = COALESCE(NULLIF(btrim(EXCLUDED.title), ''), public.revision_problems.title),
        ...
        updated_at = NOW(),
        focus_history = COALESCE(public.revision_problems.focus_history, '[]'::jsonb) || jsonb_build_array(
          jsonb_build_object(
            'score', EXCLUDED.focus_score,
            'status', EXCLUDED.focus_status,
            'switches', EXCLUDED.switches,
            'duration', EXCLUDED.focus_duration,
            'code', EXCLUDED.code,
            'timestamp', NOW()
          )
        );
