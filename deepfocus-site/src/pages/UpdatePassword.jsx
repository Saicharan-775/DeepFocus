...
export default function UpdatePassword() {
  ...
  useEffect(() => {
    const checkSession = async () => {
      try {
        ...
        if (!session) {
          // No valid session, redirect to home or login
          navigate('/');
        }
      } catch (err) {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      setSuccess(true);
      setTimeout(() => navigate('/revision'), 3000);
    } catch (err) {
        ...
        <AnimatePresence mode="wait">
          {!success ? (
            <motion.div
              ...
              exit={{ opacity: 0, x: 10 }}
            >
              <h2 className="text-2xl font-semibold mb-2 tracking-tight">Update Password</h2>
            ...
            >
              ...
              <p className="text-gray-400 text-sm leading-relaxed px-4">Your password has been successfully updated. Redirecting to dashboard...</p>
            </motion.div>
          )}
        </AnimatePresence>
      </motion.div>
